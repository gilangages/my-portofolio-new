...
<template>
  <nav ref="navRef" class="fixed bottom-4 md:top-4 md:bottom-auto left-1/2 z-40 w-[95%] md:max-w-fit">
    <div
      class="bg-white/90 backdrop-blur-md border border-black/20 rounded-2xl md:rounded-full px-2 py-2 md:px-6 md:py-2 shadow-[0_4px_20px_rgba(0,0,0,0.05)] flex items-center transition-all duration-300 hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] overflow-hidden">
      <div class="hidden md:block font-serif font-bold text-xl tracking-tighter mr-4 border-r border-black/20 pr-4">
        A
      </div>
      ...
      <div ref="menuContainer"
        ...
        <RouterLink v-for="item in menus" :key="item.name" :to="item.href"
          class="group flex-shrink-0 flex flex-col md:flex-row items-center justify-center gap-1 md:gap-1.5 p-2 md:px-3 md:py-1.5 rounded-xl md:rounded-full text-[10px] md:text-sm font-bold transition-all duration-200 border border-transparent hover:border-black/20 active:scale-95 whitespace-nowrap"
          exact-active-class="bg-black text-white border-black/20 shadow-md md:shadow-none">
